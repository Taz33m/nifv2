...
def load_and_visualize():
    ...
    test_points = torch.FloatTensor(data[:1000, :3]).to(device)  # Move to device
    ...
    model = NIFModule.load_from_checkpoint(
        'checkpoints/epoch=9-train_loss=0.00.ckpt',
    ...
    with torch.no_grad():
        pred_normals = model(test_points).cpu().numpy()  # Move back to CPU for plotting
    test_points = test_points.cpu().numpy()  # Move back to CPU for plotting
